...
def train(train_data, val_data, source_vocab, target_vocab, source_vocab_list, target_vocab_list, no_train):
...
#          model.load_state_dict(pretrained_model)
       ...
       for epoch in range(args.num_epochs):
          ...
          for batch_idx in range(0, train_data_size, args.batch_size):
              ...
              print("   batch: %s/%s" % (batch, train_data_size/args.batch_size))
...
def test(test_data, source_vocab, target_vocab, source_vocab_list, target_vocab_list):
...
parser.add_argument('--train_dir', type=str, default='/home/lola/nn/checkpoints', # default='../model_ckpts/tree2tree/',
                    help='training directory')
parser.add_argument('--train_data', type=str, default='/home/lola/nn/models_train.json', # default='../data/CS-JS/BL/preprocessed_progs_train.json',
                    help='training data')
parser.add_argument('--val_data', type=str, default='/home/lola/nn/models_valid.json', #default='../data/CS-JS/BL/preprocessed_progs_valid.json',
                    help='training data')
parser.add_argument('--test_data', type=str, default='/home/lola/nn/models_test.json', #default='../data/CS-JS/BL/preprocessed_progs_test.json',
                    help='test data')
parser.add_argument('--load_model', type=str, default=None, # default=None
...
def main():
    ...
    source_vocab, target_vocab, source_vocab_list, target_vocab_list = data_utils.build_vocab(train_data, args.vocab_filename)
    if args.test:
        test_data = json.load(open(args.test_data, 'r'))
        test(test_data, source_vocab, target_vocab, source_vocab_list, target_vocab_list)
    else:
        val_data = json.load(open(args.val_data, 'r'))
        # print("Val data %s" % val_data)
        train(train_data, val_data, source_vocab, target_vocab, source_vocab_list, target_vocab_list, args.no_train)
