...
def prepare_data(init_data, source_vocab, target_vocab):
    data = []
    for prog in init_data:
        # print(init_data)
        source_prog = prog['source_ast']
        target_prog = prog['target_ast']
        source_prog = ast_to_token_ids(source_prog, source_vocab)
        target_prog = ast_to_token_ids(target_prog, target_vocab)
